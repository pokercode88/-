...
//社交服务代理
const Social::SocialServantPrx OuterFactoryImp::getSocialServantPrx(const int64_t uid)
{
    if (!_SocialServerPrx)
    {
...
//格式化时间
string OuterFactoryImp::GetTimeFormatOffSet(int days)
{
    string sFormat("%Y-%m-%d %H:%M:%S");
    time_t t = TNOW - days * 24 * 3600;
    auto pTm = localtime(&t);
    if (pTm == NULL)
        return "";
